...
API_ID = int(environ.get('API_ID', '20635132')) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', 'ec3cebddfc932362dba7a99bb6752391')  # API Hash from my.telegram.org
BOT_TOKEN = environ.get('BOT_TOKEN', "7753987494:AAHAHgFDjryFkcInf0_RuMAXbZLs8W-g_rI")    # Bot token from @BotFather
...
SPELL_IMG = environ.get("SPELL_IMG", "https://jpcdn.it/img/e6680b19146911fafe31587395f7ae4b.jpg")
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7712194035').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002643890670').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002599350196'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002673928898'))  # Bin channel id (make sure bot is admin)
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002599350196'))  # Premium logs channel id
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1002673928898').split()] #(make sure bot is admin)
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1003152637439')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002379102420')  # Request channel id (make sure bot is admin)
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/lolansupport')  # Support group link (make sure bot is admin)

# FORCE_SUB 
auth_req_channels = environ.get("AUTH_REQ_CHANNELS", "-1002379102420")# requst to join Channel for force sub (make sure bot is admin) only for bot ADMINS  
auth_channels     = environ.get("AUTH_CHANNELS", "-1002482748550 -1002379102420")# Channels for force sub (make sure bot is admin)
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://wonderland2000azz_db_user:EDzKCCUn1jWDzxoF@cluster0.nec4izh.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")  # MongoDB URI for the database
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/movie_zone3k') # Group link for the bot
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/mz_movie_zone3k') # Owner link for the bot
UPDATE_CHNL_LNK = environ.get('UPDATE_CHNL_LNK', 'https://t.me/mz_movie_zone3k') # Update channel link for the bot
...
CUSTOM_FILE_CAPTION = environ.get("{file_name}", f"{script.CAPTION}")   # Custom caption for files
...
if 'DYNO' in environ:
    ...
    ON_HEROKU = False
BIND_ADRESS = str(getenv('WEB_SERVER_BIND_ADDRESS', 'https://pregnant-shelbi-marcomathew68-40d5524a.koyeb.app'))
